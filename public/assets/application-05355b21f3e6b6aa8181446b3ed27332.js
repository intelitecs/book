...
(function() {
  $(function() {
    var addToDones, all, allArticlesErrorCallback, allArticlesSuccessCallback, createComment, createCommentError, createCommentSuccess, current_user, current_user_erros, deleteArticle, deleteComment, deleteComments, displayComments, displayCurrentArticle, loadArticles, processSearch, replaceBody;
    $('div.comment_managment').hide();
    all = function(resource, success, error) {
      return $.ajax({
        url: "/" + resource,
        type: 'GET',
        dataType: 'json',
    ...
    loadArticles = function() {
      return all("articles", allArticlesSuccessCallback, allArticlesErrorCallback);
    };
    $('i.article-create').click(function() {
      return $.ajax({
        url: '/articles/new',
        type: 'GET',
        dataType: 'HTML',
    ...
    replaceBody = function(response, status, xhr) {
      return $('body').html(response).find('body');
    };
    displayCurrentArticle = function() {
      if (Scope.article !== {}) {
        $("img.current").slideUp('slow', function() {
          return $(this).fadeIn();
        });
        $('img.current').attr({
    ...
    current_user = function(user, status, request) {
      return Scope.current_user = user;
    };
    current_user_erros = function(errors) {
      return console.log(errors);
    };
    displayComments = function() {
      $('div.comment_column').empty();
      if (Scope.article.comments.length > 5) {
        $('div.comment_column').css({
          'overflow': 'scroll',
          'height': '400px'
        });
      }
      if ($.isEmptyObject(Scope.article.comments)) {
        ...
        return $.each(Scope.article.comments, function(index, comment) {
          var $checkBoxDiv, $commentBodyDiv, $commentCheckbox, $commentDiv, $commentH5, $commentP;
          $commentDiv = $('<div class="row comment">');
          $commentBodyDiv = $('<div class="large-14 columns pull-1 ">').css({
            'padding': '0'
          });
          if (Scope.current_user_id !== -1 && Scope.current_user.admin === true) {
            ...
            $commentCheckbox = $("<input type='checkbox' class='check'>").css({
              'marginTop': '-4px',
              'marginLeft': '-6px'
            });
            $commentCheckbox.on('change', {
              comment: comment,
              index: index
            }, addToDones);
          ...
          return $('div.comment_column').append($commentDiv);
        });
      }
    };
    addToDones = function(event) {
      var checkbox, comment, index;
      comment = event.data.comment;
      index = event.data.index;
      checkbox = event.target;
      if (checkbox.checked === true) {
        comment.done = true;
        if (Scope.commentsdones.indexOf(comment) === -1) {
          Scope.commentsdones.unshift(comment);
        }
      ...
      Scope.commentsdones = _.filter(Scope.article.comments, function(comment) {
        return comment.done;
      });
      if (Scope.commentsdones.length === 0) {
        $('span.selection').text(" ");
        return $('div.comment_managment').hide();
      } else if (Scope.commentsdones.length === 1) {
        $('div.comment_managment').show();
        $('i.comment-update').show();
        return $('span.selection').html("<b>1</b> commentaire sélectionné");
      } else {
        $('i.comment-update').hide();
        return $('span.selection').html("<b>" + Scope.commentsdones.length + "</b> commentaires sélectionnés");
      }
    };
    deleteComment = function(comment) {
      return $.ajax({
        ...
        success: function(response, status, xhr) {
          ...
          $("p#comment" + index).removeClass('done-true');
          return $('p#comment' + index).addClass('comment_body_class');
        },
        error: function(errors) {
          return console.log(errors);
        }
    ...
    $('i.comment-delete').click(function() {
      return deleteComments();
    });
    deleteComments = function() {
      var comment, k, _ref, _ref1;
      if (!!confirm("Êtes-vous sur de supprimer? ")) {
        _ref = Scope.commentsdones;
        for (k in _ref) {
          comment = _ref[k];
          deleteComment(comment);
          Scope.commentsdones.slice(Scope.commentsdones.indexOf(comment), 1);
        }
        if (Scope.commentsdones.length === 1) {
    ...
    createCommentSuccess = function(response, status, xhr) {
    ...
    createCommentError = function(errors) {
      return console.log(errors);
    };
    $('button#create_comment').click(function(e) {
      var $body, data;
      e.preventDefault();
    ...
    $('i.article-update').click(function() {
      return $.ajax({
        ...
        success: function(data, status, request) {
          ...
          return console.log("status : " + status + " | " + request.statusText);
        },
        error: function(errors) {
          return console.log("errors occured during display the updatating form : " + errors);
        }
      });
    });
    ...
    $('i.article-delete').click(function() {
    ...
    return processSearch = function(event) {
      var article, articles, index, titles;
      articles = event.data;
      titles = [];
      ...
      return $('#article_search').autocomplete({
        source: titles,
        select: function(evt, ui) {
          Scope.article = _.findWhere(articles, {
            title: ui.item.value
          });
          return displayCurrentArticle();
        }
...
(function() {
  ...
  AbstractHttp = (function(_super) {
    __extends(AbstractHttp, _super);

    function AbstractHttp(params) {
      ...
      this.open(this.params.type, this.params.url, this.params.async, this.params.user, this.params.password);
      this.processRequest();
    }

    AbstractHttp.prototype.processRequest = function() {
...
(function() {
  var all, allUsersErrorCallback, allUsersSuccessCallback, displayUsers, dones, loadUsers;
  ...
  loadUsers = function() {
    return all("users", allUsersSuccessCallback, allUsersErrorCallback);
  };

  displayUsers = function() {
    if (Scope.users.length === 0) {
      return $('table.users_table').hide();
    } else {
      $('tbody.users_table_body').empty();
      return $.each(Scope.users, function(index, user) {
        var $checkbox, $tdEmail, $tdSelection, $tdUsername, $tr;
        $tr = $('<tr>').attr({
          'class': 'user_tr' + index
        });
        $tdSelection = $('<td>').attr({
        ...
        $tr.append($tdEmail);
        return $('tbody.users_table_body').append($tr);
      });
    }
  };

  dones = function(event) {
    var checkbox, index, k, v, _ref, _results;
    Scope.user = event.data.user;
    index = event.data.index;
    checkbox = event.target;
    if (checkbox.checked === true) {
      Scope.user.done = true;
      if (Scope.usersdones.indexOf(Scope.user) === -1) {
        Scope.usersdones.unshift(Scope.user);
      }
      $('tr.user_tr' + index).addClass('done-true');
    } else {
      Scope.user.done = false;
      if (Scope.usersdones.indexOf(Scope.user) !== -1) {
        Scope.usersdones.slice(Scope.usersdones.indexOf(Scope.user), 1);
      }
      $('tr.user_tr' + index).removeClass('done-true');
    }
    _ref = Scope.usersdones;
    _results = [];
    for (k in _ref) {
      v = _ref[k];
      _results.push(console.log(v.username));
    }
    return _results;
  };

  allUsersSuccessCallback = function(data) {
  ...
  allUsersErrorCallback = function(errors) {};
...
(function() {
  $(function() {
    var $article_management, $icon_plus, current_user, current_user_erros, user_id;
    $(document).foundation();
    ...
    $icon_plus.hover(function() {
      $(this).css({
        'visibility': 'hidden'
      });
      return $article_management.css({
        'visibility': 'visible',
        'marginLeft': '-52px'
      }).addClass("position_management_on_hover");
    });
    $article_management.mouseleave(function() {
      $(this).css({
        'visibility': 'hidden'
      });
      return $icon_plus.css({
        'visibility': 'visible'
      });
    });
    $("img.article_image").hover(function() {
      return $("div.articles_browser").css({
        'visibility': 'visible'
      });
    });
    $("div.articles_row").hover(function() {
      return $("div.articles_browser").css({
        'visibility': 'visible'
      });
    });
    ...
    return console.log('current user id = ' + Scope.current_user_id + " type = " + typeof Scope.current_user_id);
  });
